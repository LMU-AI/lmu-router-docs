...
description: Claude Code CLI 接入灵眸 API 完整指南：settings.json 配置、指定模型、国产大模型调用、启用 Claude Opus 5 / Sonnet 5 的 1M 上下文（`[1M]` 后缀）与常见报错排查。
...
faq:
  ...
  - q: 报错 503 No available accounts 是什么原因？
    a: "多半是 ~/.zshrc 或 ~/.bashrc 里配置了 ANTHROPIC_AUTH_TOKEN / ANTHROPIC_BASE_URL 等全局环境变量，覆盖了 settings.json 里的配置。把这几行从 shell 配置里删掉，或移到独立文件仅在启动 Claude Code 时 source 加载。"
---
