...
keywords:
  ...
  - sub2api usage
faq:
  - q: "为什么不能直接用 sk- 开头的 API Key 拉取使用明细？"
    a: "sk- Key 仅供网关业务调用（/v1/messages 等），不开放账号级别的明细查询。这是有意的安全设计——Key 一旦泄露不希望整本账单被脱。账号级数据必须用账号鉴权（JWT）。"
  - q: 怎么用 API 导出灵眸的 token 使用明细？
    a: "两步：先用账号密码登录接口换取 JWT，再带上该 JWT 调用 /api/v1/usage，即可拿到与控制台 /usage 页面一致的完整明细，支持分页、日期范围与按模型 ID 过滤。文档里提供了 Bash / Python / Node.js 三种语言的完整示例。"
  - q: JWT 过期了怎么办？
    a: "重新调用登录接口换取新的 JWT 即可。长期运行的导出脚本应当把「登录换 token」封装成可重复调用的函数，在收到鉴权失败时自动续期后重试。"
---
...
| `model` | string | 否 | — | 按模型 ID 过滤(如 `claude-sonnet-5`) |
