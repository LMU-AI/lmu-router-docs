...
<Intl>
<Callout type="info" title="Direct overseas access via the Singapore node">

The LMU AI overseas gateway runs on a **Singapore node**, so `api.lmuai.ai` can be called directly from networks outside mainland China with **no extra proxy required**. A direct connection gives the fastest, most stable results; conversely, a proxy that rotates the exit IP tends to cause stream disconnects.
...
<Intl>
**No.** The LMU AI overseas gateway runs on a **Singapore node**, and `api.lmuai.ai` can be called directly from networks outside mainland China with no VPN or proxy.
