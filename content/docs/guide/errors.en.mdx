...
This classification comes from the retry guidance in the image APIs — see [Gemini Image · Retry guidance](/docs/api/gemini-image#重试建议).

</Callout>

Full error descriptions for each API: [Gemini Image](/docs/api/gemini-image), [GPT Image](/docs/api/gpt-image), [Grok Image](/docs/api/grok-image), [Gemini Batch Image](/docs/api/gemini-image-batch).

---

## Business error codes (batch image)

Beyond HTTP status codes, the [Gemini Batch Image API](/docs/api/gemini-image-batch) also returns business error codes, shown in the console as `error code: BATCH_IMAGE_XXX` plus a request ID.
...
The [Usage export API](/docs/api/usage-export) uses JWT authentication, so its error semantics differ from the API-key channel above:
...
- Protection against a stolen key is in [Key Security](/docs/guide/key-security)
