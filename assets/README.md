...
`app/opengraph-image.tsx` 用的字体子集，**只包含那张 OG 图上出现的字形**（约 6 KB）。
...
```bash
# 这几个字符串必须与 app/opengraph-image.tsx 里的文本逐字一致
# （SITE_NAME 来自 lib/site.ts）。漏掉哪个字符，那个字符就会回落到
# @vercel/og 自带的 Geist（无 CJK），字重和字形都对不上。
TXT=$(python3 -c "
import urllib.parse
s='灵眸文档' 'Claude Code / Codex CLI / Cursor' '一站式 AI API 接入指南' 'docs.lmuai.com'
...
验证：`npm run build` 后把 `.next/server/app/opengraph-image.body` 改名成 `.png` 打开，
确认中文正常显示、没有 □ 方块。
